...
import { Search, MapPin, Star, Filter, ShieldAlert, MessageCircle, Wallet } from "lucide-react"
...
export default function Feed() {
    ...
    useEffect(() => {
        const delayDebounceFn = setTimeout(() => {
            if (search !== undefined) { loadFeed(); setVisibleCount(10); } // Скидаємо лічильник при пошуку
        }, 500)
    ...
    const handleResetFilters = () => {
        setSearch(""); setFilterType("all"); setCityId("0");
    ...
    const FeedSkeleton = () => (
        <div className="mb-6 bg-white dark:bg-slate-900 rounded-xl shadow-md border border-slate-100 dark:border-slate-800 animate-pulse overflow-hidden">
            <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-2xl bg-slate-200 dark:bg-slate-800"></div>
                        <div className="space-y-2"><div className="h-4 w-32 bg-slate-200 dark:bg-slate-800 rounded"></div><div className="h-3 w-24 bg-slate-200 dark:bg-slate-800 rounded"></div></div>
                    </div>
                </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-800/50 p-4 border-t border-slate-100 dark:border-slate-800 flex justify-between">
                <div className="h-8 w-20 bg-slate-200 dark:bg-slate-800 rounded"></div><div className="h-10 w-32 bg-slate-200 dark:bg-slate-800 rounded-xl"></div>
            </div>
        </div>
    )
                ...
                <CardContent className="p-0">
                    <div className="p-6">
                        ...
                        <div className="mb-4">
                            ...
                            {skill.user_id !== myId && (
                                <button onClick={() => setReportData({type: 'skill', id: skill.skill_id})} className="text-slate-400 hover:text-red-500 text-xs mt-3 flex items-center gap-1 font-medium transition-colors">
                                    🚩 Поскаржитися
                                </button>
                            )}
                        </div>
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                        ...
                        {skill.user_id !== myId ? (
                            <div className="flex items-center gap-2">
                                <Button variant="outline" onClick={() => navigate(`/chat/${skill.user_id}`)} className="border-indigo-200 text-indigo-600 hover:bg-indigo-50 dark:border-indigo-900/50 dark:text-indigo-400 dark:hover:bg-indigo-900/30 font-bold rounded-xl px-4 shadow-sm">
                                    <MessageCircle size={18} className="md:mr-2" /><span className="hidden md:inline">Написати</span>
                                </Button>
                            ...
                            <Button variant="outline" disabled className="font-bold border-slate-200 dark:border-slate-700 rounded-xl">Ваше оголошення</Button>
                        )}
                    </div>
                </CardContent>
            </Card>
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-amber-700 dark:text-amber-400 leading-relaxed">
                                Задля вашої безпеки <strong>просимо проводити уроки виключно онлайн</strong> через
                                платформу (Google Meet). Не погоджуйтеся на зустрічі наодинці, не переходьте за
                                невідомими посиланнями в чаті та не діліться точною адресою проживання чи фінансовими
                                даними. Якщо ви помітили підозрілу поведінку, негайно повідомте підтримку.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="animate-in fade-in duration-500">
                    {activeTab === 'all' && (
                        loading ? (
                            ...
                            <>
                                <div>{displayedFeed.map(renderSkillCard)}</div>
